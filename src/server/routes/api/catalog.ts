...
import { getCachedCategories, getCachedBranches } from '../../../services/sufrahApi';
...
export async function handleCatalogApi(req: Request, url: URL): Promise<Response | null> {
  ...
  if (url.pathname === '/api/catalog/categories' && req.method === 'GET') {
    ...
    try {
      const categories = await getCachedCategories(restaurant.externalMerchantId);
  ...
  if (url.pathname === '/api/catalog/branches' && req.method === 'GET') {
    ...
    try {
      const branches = await getCachedBranches(restaurant.externalMerchantId);
  ...
  if (url.pathname === '/api/catalog/sync-status' && req.method === 'GET') {
    ...
    try {
      await getCachedCategories(restaurant.externalMerchantId);
